...
newtype MInterval e s = MInterval { unMInterval :: Interval (e s) }
...
instance (CanBeMutable e) => CanBeMutable (Interval e) where
    ...
    makeMutable (Interval l h) = 
        ...
        lM <- makeMutable l
        hM <- makeMutable h
        return $ MInterval $ Interval lM hM
    unsafeMakeMutable (Interval l h) = 
        do
        lM <- unsafeMakeMutable l
        hM <- unsafeMakeMutable h
        return $ MInterval $ Interval lM hM
    writeMutable (MInterval (Interval lM hM)) (Interval l h) =
        do
        writeMutable lM l
        writeMutable hM h
    unsafeWriteMutable (MInterval (Interval lM hM)) (Interval l h) =
        do
        unsafeWriteMutable lM l
        unsafeWriteMutable hM h
    readMutable (MInterval (Interval lM hM)) =
        do
        l <- readMutable lM 
        h <- readMutable hM 
        return $ Interval l h
    unsafeReadMutable (MInterval (Interval lM hM)) =
        do
